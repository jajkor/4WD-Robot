...
class GUI_App:
	...
	def __init__(self, stream_url):
		...
		self.feed_label = Label(self.root, width=480, height=320)
		self.feed_label.grid(row=0, rowspan=3, column=0, sticky=W, padx=2, pady=2)

		self.start_button = Button(self.root, text='Start Recording', command=self.start_recording)
		self.start_button.grid(row=0, column=1, sticky=NE, padx=2, pady=2)

		self.stop_button = Button(self.root, text='Stop Recording', command=self.stop_recording, state=DISABLED)
		self.stop_button.grid(row=1, column=1, sticky=E, padx=2, pady=2)

		self.recording = False
		self.video_writer = None

		self.capture_button = Button(self.root, text='Capture', command=self.capture_image)
		self.capture_button.grid(row=2, column=1, sticky=SE, padx=2, pady=2)
